...
const getWeatherData = async () => {
  // the try-catch block to handle errors
  try {
    // a const that stores the user input from the searchbar or defaults back to 'Kolkata' if left blank
    const city = searchInput.value || "Kolkata";

    // 2 promises that call the APIs and pass in the city name
    // If the user haven't typed anything, use Kolkata as default
